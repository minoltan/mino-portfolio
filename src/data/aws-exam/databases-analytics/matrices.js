...
const matrices = [
    ...
    },
    {
        ...
        subtitle: "High availability vs read scaling — the most commonly confused RDS concepts in SAA-C03",
        color: ACCENT.teal,
        columns: ["Feature", "Multi-AZ", "Read Replica"],
        rows: [
            ["Primary purpose", "High availability & automatic failover", "Read scaling & performance offload"],
            ["Replication type", "Synchronous (every write must be confirmed by standby)", "Asynchronous (slight replication lag possible)"],
            ["Readable?", "No — standby is passive; cannot serve read traffic", "Yes — separate endpoint for SELECT queries"],
            ["Failover?", "Yes — automatic DNS flip in 60–120 s (RDS) / ~30 s (Aurora)", "No — manual promotion required; not automatic"],
            ["Number of standbys", "1 synchronous standby (RDS); 6-copy storage across 3 AZs (Aurora)", "Up to 5 (RDS) / up to 15 (Aurora)"],
            ["Cross-region support", "No — Multi-AZ stays within the same region", "Yes — cross-region Read Replicas supported (async)"],
            ["Cost", "~2× instance cost (primary + standby)", "Additional instance cost per replica"],
            ["Use case", "Production DB needing < 2 min RTO; compliance requiring automatic failover", "Read-heavy workloads (reporting, analytics, dashboards)"],
            ["Data consistency", "Synchronous — zero data loss on failover", "Eventual — replica may lag behind primary by seconds"],
            ["Can be promoted?", "Not promoted — standby becomes primary only on failover", "Yes — can be promoted to standalone DB (manual)"],
            ["Endpoint change on failover?", "No — same CNAME DNS endpoint; connection string unchanged", "No endpoint change; promotion creates a new independent DB"],
            ["Backup taken from?", "Automated backups taken from standby (no I/O impact on primary)", "Backups taken from primary (replica itself can have backups enabled)"],
            ["Engine support", "All RDS engines + Aurora", "All RDS engines + Aurora (Aurora uses shared storage, not true async replication)"],
            ["Exam trigger keywords", "\"automatic failover\", \"high availability\", \"no data loss\", \"resilience\"", "\"read-heavy\", \"offload reporting\", \"scale reads\", \"analytics queries\""],
        ],
    },
    {
